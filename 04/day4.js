...
function isValid(pass) {
    ...
    for (let i=0; i<words.length; i++) {
        ...
        for (let j=0; j<words.length; j++) {
            const w2 = words[j]
            if (i !== j && isAnagram(w1, w2)) {
                console.log('anagram:', w1, 'and', w2)
                valid = false
                break
            }
        }
    }
    console.log(`${pass} is ${valid ? 'valid' : 'INVALID.'}`)
    console.log('---\n')
    return valid
}

console.assert(isValid('abcde fghij') === true)
console.assert(isValid('abcde xyz ecdab') === false)
console.assert(isValid('a ab abc abd abf abj') === true)
console.assert(isValid('iiii oiii ooii oooi oooo') === true)
console.assert(isValid('oiii ioii iioi iiio') === false)
